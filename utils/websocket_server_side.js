...
module.exports = function (cp, fcw, logger) {
    ...
    ws_server.broadcast_state = function () {
        ...
        }														//this is expected to fail for "checking"
    };
    ...
    ws_server.process_msg = function (ws, data) {
        ...
        else if (data.type === 'initAccount') {
            logger.info('[ws] initAccount req');
            options.args = {
                id: data.id,
                value: data.value,
            };
        ...
        // transferAccounts
        else if (data.type === 'transferAccounts') {
            logger.info('[ws] transferAccounts req');
            options.args = {
                from: data.from,
                to: data.to,
                value: data.value,
            ...
            marbles_lib.transfer_accounts(options, function (err, resp) {
                if (err != null) send_err(err, data);
                else options.ws.send(JSON.stringify({msg: 'tx_step', state: 'finished'}));
            });
        }
        // queryAccount
        else if (data.type === 'queryAccount') {
            logger.info('[ws] queryAccount req');
            options.args = {
                id: data.id,
            };

            marbles_lib.query_account(options, function (err, resp) {
                if (err != null) send_err(err, data);
                else options.ws.send(JSON.stringify({msg: 'account', data: resp}));
            });
        }
        // transfer a marble
